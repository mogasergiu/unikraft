...
ELF_MAGIC = {
    'i386':
    b'\x7fELF\x01\x01\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x02\x00\x03\x00',
    'x86_64':
    ...
    'arm64':
    b'\x7fELF\x02\x01\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x02\x00\xb7\x00'
}
...
class Elf64_to_32:
    ...
    def __init__(self, elf64_file):
        ...
        self.e_phoff = unpack(self.endian + 'Q', self.elf64_ehdr[32:40])[0]
        self.e_shoff = unpack(self.endian + 'Q', self.elf64_ehdr[40:48])[0]
        ehdr64_e_phentsize = unpack(self.endian + 'h', self.elf64_ehdr[54:56])[0]
        self.e_phnum = unpack(self.endian + 'h', self.elf64_ehdr[56:58])[0]
        ehdr64_e_shentsize = unpack(self.endian + 'h', self.elf64_ehdr[58:60])[0]
        self.e_shnum = unpack(self.endian + 'h', self.elf64_ehdr[60:62])[0]
    ...
    def get_elf32_ehdr(self):
        elf32_ehdr = bytearray(ELF64_EHDR_LEN)
        elf32_em = ELF_MACHINE[self.arch32]
        _endian = 'big' if self.endian == '>' else 'little'

        elf32_ehdr[0:16] = ELF_MAGIC[self.arch32]
        elf32_ehdr[16:18] = self.elf64_ehdr[16:18]                 # e_type
        elf32_ehdr[18:20] = elf32_em.to_bytes(2, _endian)          # e_machine
        elf32_ehdr[20:24] = self.elf64_ehdr[20:24]                 # e_version
        elf32_ehdr[24:28] = self.elf64_ehdr[24:28]                 # e_entry
        elf32_ehdr[28:32] = self.elf64_ehdr[32:36]                 # e_phoff
        elf32_ehdr[32:36] = self.elf64_ehdr[40:44]                 # e_shoff
        elf32_ehdr[36:40] = self.elf64_ehdr[48:52]                 # e_flags
        elf32_ehdr[40:42] = ELF32_EHDR_LEN.to_bytes(2, _endian)    # e_ehsize
        elf32_ehdr[42:44] = ELF32_PHDR_LEN.to_bytes(2, _endian)    # e_phentsize
        elf32_ehdr[44:46] = self.elf64_ehdr[56:58]                 # e_phnum
        elf32_ehdr[46:48] = ELF32_SHDR_LEN.to_bytes(2, _endian)    # e_shentsize
    ...
    def get_elf32_phdrs(self):
        def elf64_to_32_phdr(elf64_phdr):
            ...
            elf32_phdr[4:8] = elf64_phdr[8:12]                     # p_offset
    ...
    def get_elf32_shdrs(self):
        def elf64_to_32_shdr(elf64_shdr):
            ...
            elf32_shdr[16:20] = elf64_shdr[24:28]                  # sh_offset
            elf32_shdr[20:24] = elf64_shdr[32:36]                  # sh_size
            elf32_shdr[24:] = elf64_shdr[40:44]                    # sh_link
...
def main():
    ...
    # Create the new file as a duplicate which we will then overwrite
    with open(opt.elf64_file, 'r+b') as elf64_file,                           \
         open(opt.output, 'wb') as elf32_file:
        elf32_file.write(elf64_file.read())

    with open(opt.output, 'r+b') as elf32_file:
        # Zero out initial Elf64_Ehdr and return to the start of the file
        elf32_file.write(bytes(ELF64_EHDR_LEN))
        elf32_file.seek(0)

        # Overwrite with Elf32_Ehdr
        elf32_file.write(elf32_ehdr)

        # Overwrite Elf64_Phdr's with with the equivalent Elf32_Phdr's
        elf32_file.seek(elf64.e_phoff)
        for p in elf32_phdrs:
            elf32_file.write(p)

        # Overwrite Elf64_Shdr's with with the equivalent Elf32_Shdr's
        elf32_file.seek(elf64.e_shoff)
