...
# section respectively (see include/uk/uk_reloc.h)
BASE_ADDR = 0x0
...
# Return a dictionary only with the relevant fields described above
# In the case of x86, we must take care in avoiding the start16_ symbols
...
x86_ignore_sym_substring = 'start16_'
...
def rela_to_uk_reloc(rela):
    ...
    # need for a UK_RELOC_FLAGS_PHYS_REL flag (see include/uk/uk_reloc.h).
...
# provided (see include/uk/uk_reloc.h).
...
# Optionally, a _phys suffix to represent a UK_RELOC_FLAGS_PHYS_REL relocation,
# any other suffix is ignored and taken as without UK_RELOC_FLAGS_PHYS_REL).
UK_RELOC_FLAGS_PHYS_REL = (1 << 0)
...
# See include/uk/uk_reloc.h.
def build_uk_relocs(elf, rela_dyn_secs):
    ...
    uk_relocs += [uk_reloc_sym_to_struct(elf, s) for s in uk_reloc_syms]
...
def main():
    ...
    # Only operate on the debug image, as it has everything we need.
    elf_dbg = opt.elf + ".dbg"
    ...
    global BASE_ADDR
    BASE_ADDR = int(get_nm_syms(elf_dbg, r'_base_addr')[0][0], 16)

    # Make sure that all 3 obligatory sections are present for this to work
    shdrs = get_shdrs(elf_dbg)
    rela_dyn_shdr = None
    text_shdr = None
    dyn_shdr = None
    sh_found = 0
    ...
    if dyn_shdr == None or rela_dyn_shdr == None or text_shdr == None:
        raise Exception("Error while looking for .dynamic, .rela.dyn "
                        "and .text Section Headers!")
    if sh_found > 3:
        raise Exception("Found more than 3 of the required sections: "
                        ".dynamic, .rela.dyn, .text. Are there sections with "
                        "the same names?")

    # Get the RELACOUNT value to compare against found entries in .rela.dyn
    # as a sanity check.
    dyn_secs = get_dyn_secs(elf_dbg)
    relacount = 0
    for d in dyn_secs:
        if d['Type'] == 'RELACOUNT':
    ...
    rela_dyn_secs = get_rela_dyn_secs(elf_dbg)
    ...
    uk_relocs = build_uk_relocs(elf_dbg, rela_dyn_secs)

    # Write the binary blob with `struct uk_reloc` entries
    with open(opt.elf + '_uk_reloc.bin', 'wb') as bin:
        bin.write(UK_RELOC_SIGNATURE.to_bytes(4, endianness))      # signature

        def write_uk_reloc_to_bin(ur):
            bin.write(ur[0].to_bytes(8, endianness))               # m_off
            bin.write(ur[1].to_bytes(8, endianness, signed=True))  # r_val_off
            bin.write(ur[2].to_bytes(4, endianness))               # r_sz
            bin.write(ur[3].to_bytes(4, endianness))               # flags

        for ur in uk_relocs:
            write_uk_reloc_to_bin(ur)

        # Now write the sentinel (a zeroed out entry, but only r_sz is required
        # to be 0 anyway)
        write_uk_reloc_to_bin([0, 0, 0, 0])

    # One last step, statically resolve every relocation by patching the binary
    # with the supposed absolute value of the symbols. This enables the kernel
    # to still work, even without a relocator (assuming we are not loaded at a
    # random physical address and KASLR is not activated of course).
    text_f_off = int(text_shdr['Offset'], 16)
    with open(opt.elf, 'r+b') as elf:
        for ur in uk_relocs:
            # Location in file is relative to .text's offset in file
            elf.seek(ur[0] + text_f_off)
            elf.write((ur[1] + BASE_ADDR).to_bytes(ur[2], endianness))
