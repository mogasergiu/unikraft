BEGIN {
	...
	print "UK_SYSCALL_USR_PROLOGUE_DEFINE(uk_syscall6_r, uk_syscall6_r_u,"
	print "\t\t\t\t14, long, nr, long, arg1, long, arg2, long, arg3, long, arg4, long, arg5, long, arg6)";
	print "\nlong __used uk_syscall6_r_u(struct uk_syscall_regs *usr)"
	print "{"
	print "\tlong ret;"
	print "\n\tswitch (usr->regs.rsyscall) {"
}
...
/[a-zA-Z0-9]+-[0-9]+/{
	...
	printf "\t\tret = %s((long)usr);\n", uk_syscall_r_u;
	printf "#else /* !HAVE_uk_syscall_u_%s */\n", name;
	printf "\t\tret = %s(\n\t\t\t\t\t", uk_syscall_r;
	for (i = 0; i < args_nr - 1; i++)
		printf("usr->regs.rarg%d, ", i)
	if (args_nr > 0)
		printf("usr->regs.rarg%d", args_nr - 1)
	printf(");\n")
