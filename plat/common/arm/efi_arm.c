...
void __no_pauth _libkvmplat_entry(void *dtb_pointer);
void lcpu_start(void *);
extern __paddr_t vector_table;

static __u8 uk_efi_bootstack[__PAGE_SIZE];

void lcpu_start_exit()
{
	_libkvmplat_start(NULL);
}
...
{
	struct ukplat_memregion_desc *dtb_mrd;
