...
def main():
    ...
    # Make sure that all 4 obligatory sections are present for this to work
    ...
    if sh_found != 4:
        raise Exception("Found more or less than 4 of the required sections: "
                        ".dynamic, .rela.dyn, .text, .uk_reloc. Are there "
    ...
    uk_reloc_end = int(get_nm_syms(opt.elf, r'__bss_start')[0][0], 16)
    uk_reloc_sz = 24
    ...
    with open(opt.elf + '_uk_reloc.bin', 'wb') as ur_bin:
        ur_bin.write(UK_RELOC_SIGNATURE.to_bytes(8, endianness))      # signature

        def write_uk_reloc_to_ur_bin(ur):
            ur_bin.write(ur[0].to_bytes(8, endianness))               # m_off
            ur_bin.write(ur[1].to_bytes(8, endianness, signed=True))  # r_val_off
